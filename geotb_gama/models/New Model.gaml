...
//		save adm3 crs: 3857 to: "../includes/adm3_hanoi.shp" type: "shp" attributes:["ID"::int(self), "NAME_1"::NAME_1, "GID_1"::GID_1, "NAME_2"::NAME_2, "GID_2"::GID_2, "NAME_3"::NAME_3, "GID_3"::GID_3, "VARNAME_1"::VARNAME_1, "VARNAME_2"::VARNAME_2, "VARNAME_3"::VARNAME_3];
	...
	action ss {
		string fpath <- "../includes/1GeoSp_VTM_KhamBenh_16_20.csv";
	...
	action ssss {
		adm3 a3 <- alladm3["" + (data1[2, idx]) + (data1[3, idx]) + (data1[4, idx])];
		adm2 a2 <- alladm2["" + (data1[2, idx]) + (data1[3, idx])];
		if (a2 != nil) {
			if (a3 != nil) {
				ask a3 {
					risk_point <- risk_point + 1;
					my_risk_color <- hsb((1 - ((risk_point > max_risk_point ? max_risk_point : (risk_point)) / max_risk_point)) * (0.17), 1, 1);
				}
