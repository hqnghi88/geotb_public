...
global {
	...
	//		file patient_csv_file <- csv_file("../includes/data/GeoSp_VTM_KhamBenh_16_20_gender_age.csv", ";", true);
		...
		file patient_csv_file1 <- csv_file("../includes/data/BN_KhangThuoc_2016_2020.csv", ";", true);
		data3 <- ((patient_csv_file1.contents));
	...
	action mdr {
		...
		l2 <- "";
		l3 <- "";
		date tmp <- date(data3[10, idx2]);
		string ttime <- "" + tmp.year + "-" + tmp.month; // + "-" + tmp.day;
		do cleaning;
		if (l1 != "" and alladm1[l1 + "saht" + (32)] != nil) {
			alladm1[l1 + "saht" + (32)][ttime] <- alladm1[l1 + "saht" + (32)][ttime] + 1;
		} else {
			write l1 + "saht" + (32);
		}
