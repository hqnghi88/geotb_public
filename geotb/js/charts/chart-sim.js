...
function showchart_sim() {
    ...
    internal_show_sim(cas[0], cm_sim);
    ...
    for (var j = 0; j < headers_sim.length; j++) {
        ...
        simvn[0][j] = (simvn[0][j] * 100000 / p[j]) * ((param[0] / 5) * B2 + (1 - (param[0] / 5)) * B3) * B4 * B5 / 5000000;
    ...
    internal_show_sim_rate(simvn[0], cm0[0]);
}

function showchart_sim1(selected_adm1) {
    ...
    internal_show_sim_rate(simvn, cm0);
}



function showchart_sim2(selected_adm1, selected_adm2) {
    ...
    internal_show_sim_rate(array, cm0);
}


function internal_show_sim(data, sim) {
    if (mysimChart != null) {
        mysimChart.destroy();
    }
    var ctx = document.getElementById("mysimChart");
    mysimChart = new Chart(ctx, {
        type: 'line',
        data: {
...
function internal_show_sim_rate(simvn, cm0) {
    if (mysimRateChart != null) {
        mysimRateChart.destroy();
    }
    var ctx = document.getElementById("mysimRateChart");
    mysimRateChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: headers_sim.slice(slider_idx1, slider_idx2 * 1 + 1),// headers.slice(slider_idx1, slider_idx2 * 1 + 1),
            datasets: [{
...
// function showchart_sim3(selected_adm1, selected_adm2, selected_adm3) {
